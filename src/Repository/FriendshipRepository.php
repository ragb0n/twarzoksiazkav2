...
class FriendshipRepository extends ServiceEntityRepository
{
    ...
    public function findAllUserFriends(int $userId): array{
        $qb = $this->createQueryBuilder('f')
            ->select('IDENTITY(f.targetUserId)')
            ->where('f.sourceUserId = :userId')
            ->setParameter('userId', $userId);
        
        $sourceIds = $qb->getQuery()->getResult();

        $qb = $this->createQueryBuilder('f')
                ->select('IDENTITY(f.sourceUserId)')
                ->where('f.targetUserId = :userId')
                ->setParameter('userId', $userId);

        $targetIds = $qb->getQuery()->getResult();

        $friendIds = array_merge($sourceIds, $targetIds);
        return $friendIds;
